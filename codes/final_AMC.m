...
subplot(2,1,1)
plot(dates, prices)
title('Adjusted Closing Of AMC Over Time')

subplot(2,1,2)
plot(returnDates, df.returns)
title('Profit and Loss of AMC Over Time')
hold off
%% histogram
figure;
hist(returns,500)
xlabel('Profit and Loss for AMC')
ylabel('Observations')
...
subplot(2,1,1) 
qqplot(returns)
title('AMC P&L (Normal Distribution)')

subplot(2,1,2)
qqplot(returns, tDist)
title('AMC P&L (T Distribution, default is 5 DoF)')
...
hold off
%% Jarque-Bera test
[h,p,jbstat,critval] = jbtest(returns,0.05) % Jarque-Bera test under 95% confidence level
h_AMC=h
p_AMC=p
jbstat_AMC=jbstat
critval_AMC=critval
...
fprintf(['99%% Age Weighted ETL(lambda = 0.98):\n<strong>%f</strong>\n\n'], ageETL99)
fprintf(['99%% Age Weighted ETL(lambda = 0.96):\n<strong>%f</strong>\n\n'], ageETL99_1)
fprintf(['99%% Age Weighted ETL(lambda = 0.94):\n<strong>%f</strong>\n\n'], ageETL99_2)
...
r = length(rollingNormVaR) -1;
% we only show the last 300 days in plot
close all; hold on
plot(sampleDates(end - 300:end), sampleReturns(end - 300:end),'r-');
plot(sampleDates(end - 300:end), rollingNormVaR(end - 300:end), 'g')
plot(sampleDates(end - 300:end), rollingTVaR(end - 300:end), 'm:')
plot(sampleDates(end - 300:end), rollingCFVaR(end - 300:end),'k--')
plot(sampleDates(end - 300:end), rollingMcVaR(end - 300:end),'g--')
plot(sampleDates(end - 300:end), rollingMctVaR(end - 300:end), 'c--')
plot(sampleDates(end - 300:end), rollingExVaR(end - 300:end),'b:')
 
legend('AMC Profit and Loss', 'Normal Distribution', 'T Distribution(2 DoF)', 'Cornish-Fisher','Monte Carlo(Normal)', 'Monte Carlo (T)', 'Extreme Value','Location', 'best');
ylabel('Returns $');
title('99% 1 Day Parametric VaR Estimates & PnL For AMC');
...
plot(sampleDates(end - 300:end), sampleReturns(end - 300:end),'r-');
plot(sampleDates(end - 300:end), rollingHistVaR(end - 300:end),'m--')
plot(sampleDates(end - 300:end), rollingVolWVaR(end - 300:end),'c--')
plot(sampleDates(end - 300:end), rollingAgeWVaR(end - 300:end),'g--')
plot(sampleDates(end - 300:end), rollingVolnormalVaR(end - 300:end),'b--')
%plot(sampleDates(end - 300:end), comboVaR(end - 300:end),'b:')
ylabel('Returns $');
legend('AMC Profit and Loss', 'Historical', 'Volatility Weighted', 'Age Weighted', 'Volatility Weighted & Normal (GARCH)', 'Location', 'best');
%legend('AMC Profit and Loss', 'Historical', 'Volatility Weighted', 'Age Weighted', 'Location', 'best');
title('99% 1 Day Non-Parametric VaR Estimates & PnL For AMC');
...
ylabel('Returns $')
title('99% 1 Day Parametric ES Estimates For AMC')
...
title('99% 1 Day Non-Parametric ES Estimates For AMC')
...
ylabel('VaR')
xlabel('Date')
title('99% VaR violations for Parametric methods for AMC')
ax = gca;
...
ylabel('VaR')
xlabel('Date')
title('99% VaR violations for Non-Parametric methods for AMC')
ax = gca;
ax.ColorOrderIndex = 1;
...
vbt1 = varbacktest(sampleReturns,[rollingNormVaR rollingTVaR rollingCFVaR(2:1264,:) rollingMcVaR rollingMctVaR rollingExVaR],'PortfolioID','AMC','VaRID',...
    {'Normal Distribution' 'T Distribution(4 DoF)' 'Cornish-Fisher' 'Monte Carlo(Normal)' 'Monte Carlo (T)' 'Extreme Value'},'VaRLevel',[0.99]);
